...
class RecurrentLayerWithSearch(Layer):
    ...
    def __init__(self, rng,
                 ...
                 coverage_dim=None,
                 use_coverage_for_alignment=False,
                 ...
                 coverage_accumulated_operation=False,
                 # added by Zhaopeng Tu, 2015-11-15
                 use_recurrent_coverage=False,
                 # added by Zhaopeng Tu, 2015-11-10
                 use_recurrent_gating_coverage=False,
                 # added by Zhaopeng Tu, 2015-11-12
                 use_input_annotations_for_recurrent_coverage=False,
                 use_decoding_state_for_recurrent_coverage=False,
                 coverage_activation=TT.tanh,
                 coverage_gater_activation=TT.nnet.sigmoid,
                 coverage_reseter_activation=TT.nnet.sigmoid,
                 # ====================
                 scale=.01,
                 activation=TT.tanh,
                 bias_fn='init_bias',
        ...
        # added by Zhaopeng Tu, 2015-11-12
        if type(coverage_activation) is str or type(coverage_activation) is unicode:
            coverage_activation = eval(coverage_activation)
        if type(coverage_gater_activation) is str or type(coverage_gater_activation) is unicode:
            coverage_gater_activation = eval(coverage_gater_activation)
        if type(coverage_reseter_activation) is str or type(coverage_reseter_activation) is unicode:
            coverage_reseter_activation = eval(coverage_reseter_activation)
        ...
        self.c_dim = c_dim
        # added by Zhaopeng Tu, 2015-10-29
        self.maintain_coverage = maintain_coverage
        self.coverage_dim = coverage_dim
        
        # added by Zhaopeng Tu, 2015-11-08
        self.use_coverage_for_alignment = use_coverage_for_alignment
        self.use_coverage_for_decoding = use_coverage_for_decoding

        # added by Zhaopeng Tu, 2015-11-04
        self.use_hard_alignment = use_hard_alignment
        # added by Zhaopeng Tu, 2015-11-15
        self.use_accumulated_coverage = use_accumulated_coverage
        # added by Zhaopeng Tu, 2015-11-04
        self.use_accumulated_coverage_bound = use_accumulated_coverage_bound
        # added by Zhaopeng Tu, 2015-11-09
        self.coverage_accumulated_operation = coverage_accumulated_operation


        # added by Zhaopeng Tu, 2015-11-15
        self.use_recurrent_coverage = use_recurrent_coverage
        # added by Zhaopeng Tu, 2015-11-10
        self.use_recurrent_gating_coverage = use_recurrent_gating_coverage
        # added by Zhaopeng Tu, 2015-11-12
        self.coverage_activation = coverage_activation
        self.coverage_gater_activation = coverage_gater_activation
        self.coverage_reseter_activation = coverage_reseter_activation
        self.use_input_annotations_for_recurrent_coverage = use_input_annotations_for_recurrent_coverage
        self.use_decoding_state_for_recurrent_coverage = use_decoding_state_for_recurrent_coverage
    ...
    def _init_params(self):
        ...
        # ================================
        # added by Zhaopeng Tu, 2015-10-29
        if self.maintain_coverage and self.use_coverage_for_alignment:
            self.C_covp = theano.shared(
                    sample_weights_classic(self.coverage_dim,
                        self.n_hids,
                        -1,
                        10 ** (-3),
                        rng=self.rng),
        ...
        if self.maintain_coverage and self.use_recurrent_gating_coverage:
            self.W_cc = theano.shared(
                    self.init_fn(self.coverage_dim,
                    self.coverage_dim,
                    -1,
                    self.scale,
                    rng=self.rng),
            ...
            self.Cov_inputer_p = theano.shared(
                    numpy.zeros((1, self.coverage_dim), dtype="float32"),
            ...
            if self.use_input_annotations_for_recurrent_coverage:
            ...
            # for decoding states
            if self.use_decoding_state_for_recurrent_coverage:
    ...
    def step_fprop(self,
        ...
        # similar as c
        if self.maintain_coverage and coverage_before.ndim == 2:
            coverage_before = coverage_before[:, None, :]
        ...
        # added by Zhaopeng Tu, 2015-10-29
        if self.maintain_coverage and self.use_coverage_for_alignment:
            # this has a huge influence on the final result
            p_from_coverage = utils.dot(coverage_before, C_covp).reshape((source_len, source_num, dim))
            p += p_from_coverage
        ...
        if self.maintain_coverage and self.use_coverage_for_decoding:
            # added by Zhaopeng Tu, 2015-10-29
            ctx = (TT.concatenate((c, coverage_before), axis=2) * probs.dimshuffle(0, 1, 'x')).sum(axis=0)
        else:
        ...
        if self.maintain_coverage:
            # for hard alignment
            if self.use_hard_alignment:
                # we only set the position with highest alignment prob as 1.0, others as 0.0
                # can be treated as hard alignment
                # along source_num axis
                probs_max = TT.max(probs, axis=0)
                probs = 1.0*((probs - probs_max) >= 0)


            # updating coverage model
            if self.use_recurrent_gating_coverage:
                ...
                if self.use_input_annotations_for_recurrent_coverage:
                    if not given_cov_state_below:
                        ...
                        cov_state_below += utils.dot(c, Cov_inputer_c).reshape((source_len, source_num, self.coverage_dim))
                        cov_gater_below += utils.dot(c, Cov_updater_c).reshape((source_len, source_num, self.coverage_dim))
                        cov_reseter_below += utils.dot(c, Cov_reseter_c).reshape((source_len, source_num, self.coverage_dim))
                    else:
                ...
                # Reset gate:
                # optionally reset the hidden state.
                cov_reseter = self.coverage_reseter_activation(TT.dot(coverage_before, R_cc) +
                        cov_reseter_below)
                ...
                coverage = self.coverage_activation(preactiv_coverage)

                # Update gate:
                # optionally reject the potential new state and use the new one.
                cov_updater = self.coverage_gater_activation(TT.dot(coverage_before, G_cc) +
                        cov_gater_below)
                coverage = cov_updater * coverage + (1-cov_updater) * coverage_before
            else:
                coverage = TT.unbroadcast(probs.dimshuffle(0,1,'x'), 2)
                
                if self.coverage_accumulated_operation == 'additive':
                    # use additive coverage
                    coverage = coverage_before + coverage
                elif self.coverage_accumulated_operation == 'subtractive':
                    # use subtractive coverage
                    coverage = coverage_before - coverage
                elif self.coverage_accumulated_operation == 'max-pooling':
                    # max pooling for coverage
                    max_mask = coverage > coverage_before
                    coverage = max_mask*coverage + (1-max_mask)*coverage_before
                elif self.coverage_accumulated_operation == 'mean-pooling':
                    # mean pooling for coverage
                    # step_num starts from 0
                    weight = 1.0 / (step_num + 1.0)
                    if type(weight) == theano.tensor.TensorVariable and weight.dtype == 'float64':
                        weight  = weight.astype('float32')
                    coverage = (1.0 - weight) * coverage_before + weight * coverage
                else:
                    raise Exception("Not a valid accumulated operation: %s" % self.coverage_accumulated_operation)

                # for coverage bound
                if self.use_accumulated_coverage_bound and (self.coverage_accumulated_operation not in ['max-pooling', 'mean-pooling']):
                    # mask is only available for additive or subtractive coverage
                    # coverage mask, another tensor with the same shape as cov
                    if self.coverage_accumulated_operation == 'additive':
                        # if any elem in coverage is greater than 1.0, set it be 1.0
                        coverage_mask = coverage > 1
                        coverage = (1-coverage_mask)*coverage + coverage_mask
                    elif self.coverage_accumulated_operation == 'subtractive':
                        # if any elem in coverage is less than 0.0, set it be 0.0
                        coverage_mask = coverage < 0
                        coverage = (1-coverage_mask)*coverage


                    # note, we can also use TT.tanh for this purpose, its output range is [0,1] for the input range [0,)

            if cndim == 2:
                # for sampling and beam_search
                # here c and coverage are 2-dimension
                coverage = coverage[:,0,:]
    ...
    def fprop(self,
        ...
        if self.maintain_coverage and self.use_recurrent_gating_coverage and self.use_input_annotations_for_recurrent_coverage:
            # this is the bias for coverege
            cov_inputer_from_c = utils.dot(c, self.Cov_inputer_c).reshape((c.shape[0], c.shape[1], self.coverage_dim))
            cov_gater_from_c = utils.dot(c, self.Cov_updater_c).reshape((c.shape[0], c.shape[1], self.coverage_dim))
            cov_reseter_from_c = utils.dot(c, self.Cov_reseter_c).reshape((c.shape[0], c.shape[1], self.coverage_dim))
        
        # The general order of function parameters to fn is:
        # sequences (if any), prior result(s) (if needed), non-sequences (if any)
        if not self.maintain_coverage: 
            if mask:
            ...
            # added by Zhaopeng Tu, 2015-11-11
            if self.use_recurrent_gating_coverage and self.use_input_annotations_for_recurrent_coverage:
                # added by Zhaopeng Tu, 2015-10-30
                if mask:
                    sequences = [state_below, mask, updater_below, reseter_below]
                    non_sequences = [c, c_mask, p_from_c, cov_inputer_from_c, cov_gater_from_c, cov_reseter_from_c] 
                    #              seqs     |     out      |  non_seqs
                    fn = lambda x, m, g, r,   h, coverage,   c1, cm, pc, cx, cg, cr : self.step_fprop(x, h, mask=m,
                    ...
                    sequences = [state_below, updater_below, reseter_below]
                    non_sequences = [c, p_from_c, cov_inputer_from_c, cov_gater_from_c, cov_reseter_from_c]
                    #            seqs    |     out     | non_seqs
                    fn = lambda x, g, r,   h, coverage,  c1, pc, cx, cg, cr : self.step_fprop(x, h,
                            coverage_before=coverage,
                            given_cov_state_below=cx, given_cov_gater_below=cg, given_cov_reseter_below=cr,
                            gater_below=g, reseter_below=r,
                            c=c1, p_from_c=pc,
                ...
                # added by Zhaopeng Tu, 2015-10-30
                # modified by Zhaopeng Tu, 2015-11-16, step_number for mean-pooling
                if mask:
                    sequences = [state_below, mask, updater_below, reseter_below, TT.arange(nsteps, dtype="int64")]
                    non_sequences = [c, c_mask, p_from_c] 
                    #              seqs        |     out      |  non_seqs
                    fn = lambda x, m, g, r, s,   h, coverage,   c1, cm, pc : self.step_fprop(x, h, mask=m,
                            coverage_before=coverage,
                            # added by Zhaopeng Tu, 2015-11-16
                            step_num = s,
                            gater_below=g, reseter_below=r,
                            c=c1, p_from_c=pc, c_mask=cm,
                            use_noise=use_noise, no_noise_bias=no_noise_bias,
                    ...
                    sequences = [state_below, updater_below, reseter_below, TT.arange(nsteps, dtype="int64")]
                    non_sequences = [c, p_from_c]
                    #            seqs       |     out     | non_seqs
                    fn = lambda x, g, r, s,   h, coverage,  c1, pc : self.step_fprop(x, h,
                            coverage_before=coverage,
                            # added by Zhaopeng Tu, 2015-11-16
                            step_num = s,
                            gater_below=g, reseter_below=r,
        ...
        if self.maintain_coverage:
            # init_coverage = TT.zeros((c.shape[0], c.shape[1], self.coverage_dim), dtype='float32')
            ...
            # When we pass outputs_info to scan, there should essentially be no broadcastable dimension.
            # Added a broadcastable option to tensor.alloc. When broadcatable=False, no axis will be set broadcastable even when its dim is 1. The default value is True.
            ...
            if self.use_accumulated_coverage and self.coverage_accumulated_operation == 'subtractive':
                init_coverage = TT.unbroadcast(TT.ones((c.shape[0], c.shape[1], self.coverage_dim), dtype='float32'), 2)
            else:
                init_coverage = TT.unbroadcast(TT.zeros((c.shape[0], c.shape[1], self.coverage_dim), dtype='float32'), 2)
...
class EncoderDecoderBase(object):
    ...
    def _create_transition_layers(self):
        ...
        if rec_layer == RecurrentLayerWithSearch:
            ...
            # added by Zhaopeng Tu, 2015-10-30
            add_args['maintain_coverage'] = self.state['maintain_coverage']
            add_args['coverage_dim'] = self.state['coverage_dim']
            # added by Zhaopeng Tu, 2015-11-08
            add_args['use_coverage_for_alignment'] = self.state['use_coverage_for_alignment']
            add_args['use_coverage_for_decoding'] = self.state['use_coverage_for_decoding']

            add_args['use_hard_alignment'] = self.state['use_hard_alignment']

            # added by Zhaopeng Tu, 2015-11-05
            add_args['use_accumulated_coverage'] = self.state['use_accumulated_coverage']
            # added by Zhaopeng Tu, 2015-11-04
            add_args['use_accumulated_coverage_bound'] = self.state['use_accumulated_coverage_bound']
            # added by Zhaopeng Tu, 2015-11-09
            add_args['coverage_accumulated_operation'] = self.state['coverage_accumulated_operation']

            # added by Zhaopeng Tu, 2015-11-15
            add_args['use_recurrent_coverage'] = self.state['use_recurrent_coverage']
            # added by Zhaopeng Tu, 2015-11-11
            add_args['use_recurrent_gating_coverage'] = self.state['use_recurrent_gating_coverage']
            # added by Zhaopeng Tu, 2015-11-12
            add_args['coverage_activation'] = self.state['coverage_activation']
            add_args['coverage_gater_activation'] = self.state['coverage_gater_activation']
            add_args['coverage_reseter_activation'] = self.state['coverage_reseter_activation']
            add_args['use_input_annotations_for_recurrent_coverage'] = self.state['use_input_annotations_for_recurrent_coverage']
            add_args['use_decoding_state_for_recurrent_coverage'] = self.state['use_decoding_state_for_recurrent_coverage']
        for level in range(self.num_levels):
...
class Decoder(EncoderDecoderBase):
    ...
    def _create_decoding_layers(self):
        ...
        # added by Zhaopeng Tu, 2015-10-30
        # here we judege whether we need coverage
        n_in=self.state['c_dim']
        if self.state['search'] and (self.state['maintain_coverage'] and self.state['use_coverage_for_decoding']):
            n_in += self.state['coverage_dim']
    ...
    def _create_readout_layers(self):
        ...
        # added by Zhaopeng Tu, 2015-11-01
        # here we judege whether we need coverage
        n_in=self.state['c_dim']
        if self.state['search'] and (self.state['maintain_coverage'] and self.state['use_coverage_for_decoding']):
            n_in += self.state['coverage_dim']
        ...
        if self.state['deep_out']:
            ...
            self.output_layer = eval(softmax_layer)(
                    ...
                    use_coverage_cost = self.state['use_coverage_cost'],
    ...
    def build_decoder(self, c, y,
        ...
        if mode == Decoder.EVALUATION:
            ...
            if self.state['maintain_coverage']:
                ...
                if self.state['use_accumulated_coverage']:
                    assert self.state['coverage_dim'] == 1
                # added by Zhaopeng Tu, 2015-11-29
                if self.state['use_coverage_cost']:
                    # now we only use the coverage cost for accumulated coverage
                    assert self.state['use_accumulated_coverage']
                assert (self.state['use_accumulated_coverage'] or self.state['use_recurrent_coverage'])
                assert not (self.state['use_accumulated_coverage'] and self.state['use_recurrent_coverage'])
        ...
        if mode == Decoder.SAMPLING:
            ...
            if self.state['use_coverage_cost']:
                return (self.output_layer.train(
                        ...
                        target=y,
                        mask=y_mask,
                        # coverage contains all time-series
                        coverages=coverage,
                        c_mask=c_mask,
                        reg=None),
                        alignment)
    ...
    def sampling_step(self, *args):
        ...
        if self.state['maintain_coverage']:
            hidden_states = self.build_decoder(y=sample, step_num=step_num, mode=Decoder.SAMPLING, **decoder_args)[2:-1]
            coverage = self.build_decoder(y=prev_word, step_num=step_num, mode=Decoder.SAMPLING, **decoder_args)[-1]
            return [sample, log_prob] + hidden_states + [coverage]
        else:
    ...
    def build_next_probs_predictor(self, c, step_num, y, init_states, coverage_before=None):
        return self.build_decoder(c, y, mode=Decoder.BEAM_SEARCH,
                given_init_states=init_states, step_num=step_num, coverage_before=coverage_before)

    def build_next_states_computer(self, c, step_num, y, init_states, coverage_before=None):
        return self.build_decoder(c, y, mode=Decoder.SAMPLING,
                given_init_states=init_states, step_num=step_num, coverage_before=coverage_before)[2:]
...
class RNNEncoderDecoder(object):
    ...
    def build(self):
        ...
        if self.state['maintain_coverage']:
            (self.sample, self.sample_log_prob, self.sample_coverage), self.sampling_updates =\
        ...
        self.coverage_before = TT.matrix("coverage_before")
    ...
    def create_sampler(self, many_samples=False):
        ...
        if self.state['maintain_coverage']:
            outputs = [self.sample, self.sample_log_prob, self.sample_coverage]
        else:
    ...
    def create_next_probs_computer(self):
        if not hasattr(self, 'next_probs_fn'):
            self.next_probs_fn = theano.function(
                    inputs=[self.c, self.step_num, self.gen_y] + self.current_states + [self.coverage_before],
                    outputs=[self.decoder.build_next_probs_predictor(
                        self.c, self.step_num, self.gen_y, self.current_states, self.coverage_before)],
                    name="next_probs_fn")
