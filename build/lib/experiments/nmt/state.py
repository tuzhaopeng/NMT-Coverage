...
def prototype_search_with_coverage_state():
    ...
    state['use_coverage_cost'] = False

    state['coverage_dim']=1
    # Hard Coverage: at each step, set the align of highest probability to be 1.0, and other aligns to be 0.0; default is soft coverage that uses the real probabilities of aligns
    state['use_hard_alignment']=False
    
    state['use_coverage_for_alignment']=True
    state['use_coverage_for_decoding']=False

    # for simple coverage, we direct operate on alignment probabilities only
    state['use_accumulated_coverage'] = True
    state['use_accumulated_coverage_bound']=False
    # we define 3 types of accumulated operation, for each position
    ...
    # Hidden layer configuration for the coverage
    state['coverage_rec_gating'] = True
    state['coverage_rec_reseting'] = True
    # not reflected in the code, we use the same activation as decoding
    # activation function
    state['coverage_activation'] = 'lambda x: TT.tanh(x)'

    state['coverage_gater_activation'] = 'lambda x: TT.nnet.sigmoid(x)'
    state['coverage_reseter_activation'] = 'lambda x: TT.nnet.sigmoid(x)'
